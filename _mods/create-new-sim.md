---
mod_id: create-new-sim
title: "Create New Sim"
icon: "fas fa-plus-circle"
requirements: ["lot51_core"]
categories: [Cheats & Debugging, Sim Personalization]
---

## Mod Description
The **Create New Sim** mod unlocks and reorganizes hidden debug interactions, allowing you to spawn new Sims and animals instantly. Originally developed as a testing tool, it is now a powerful utility for players who want to populate their world without going through the CAS process.

Simply **Shift-Click on the floor** in Live Mode to access the new menu.

### 🐾 Instant Spawning
With just one click, you can create:
* **Humans & Occults:** Quickly add new townies to your current lot.
* **Pets:** Dogs, Cats, and even Foxes (requires *EP Cats & Dogs* or *EP Cottage Living*).
* **Livestock:** Horses and small farm animals (requires corresponding Packs).
* **Robots:** Servo and other robotic entities (requires *EP Discover University*).

### 🚀 Key Features
* **Visual Overhaul:** Reorganized menus with intuitive icons to help you find the right creature type instantly.
* **Pure Scripting:** This mod does not overwrite any EA files, ensuring maximum compatibility.
* **Lightweight:** Highly optimized for a seamless experience.

---

## Technical Information

### 🌍 Translations
No separate translations are required! This mod exclusively uses strings already present in the game's code, so it will automatically appear in your game's language (English, German, Russian, Spanish, etc.).

### 🛠️ Requirements
* **Lot51 Core Library** is strictly required for the menu to function.
* **Expansion Packs:** While the mod is Base Game compatible, you will only see options for animals (like Horses or Foxes) if you own the required DLC.
* **Script Mods** must be enabled in your game settings.

---

## Developer's Note
> "When testing some of my mods, I always needed new 'freshly created' Sims, so I created this mod to save time. It's the perfect companion for anyone who manages large worlds or needs quick test subjects."
> — *Andirz*

---
mod_id: x_mas_collection
title: "X-Mas Painting Collection"
icon: "fas fa-holly-berry"
categories: [Buy Mode, Decor, Christmas]
---

## Mod Description

Keep the Christmas spirit alive in your Sims 4 game with this festive collection of Christmas-themed paintings and posters! This mod offers a variety of Christmas decorations to bring joy and a festive atmosphere to your Sims' homes. All items are purely decorative and can be found in Build Mode.

### What’s included in this mod?

* **Vol 1: Small Christmas Posters** – available in 12 different swatches.
* **Vol 2: Big Christmas Posters** – available in 12 different swatches.
* **Vol 3: Medium Christmas Paintings in Frames** – featuring 33 swatches to match any decor.
* **Vol 4: Huge Christmas Wall Motifs** – striking, large wall motifs in 12 swatches for maximum holiday spirit.
* **Exclusive Paintings** – Six exclusive Christmas paintings, each with 13 different frame colors.

## Requirements

* **Base Game Compatible** – No expansion packs or additional DLC required.
* **Purely Decorative** – Items do not contain custom scripts or gameplay interactions.

## Translations

The strings are available in the following languages:
* English (EN)
* German (DE)
* Russian (RU)
* Spanish (ES)

*Note: For all other languages, English strings are used by default. If you would like to contribute a translation, please contact the author.*

## Compatibility

There are no known compatibility issues as this mod does not overwrite any existing game resources.

## Credits & Copyright

All motifs used in this collection are taken from **Pixabay.com** under free licenses.

---

## Developer's Note
> "Celebrate the season in style! I wanted to create a collection that helps every Sim home feel a little more cozy and festive during the holidays. Merry Christmas and Happy New Year!"
> — *Andirz*